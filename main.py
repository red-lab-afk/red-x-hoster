...
TOKEN = os.getenv('TOKEN', '8750572344:AAFt6eoeyf2qux7HhKG11DAWVJt86Zi9bJI')
OWNER_ID = int(os.getenv('OWNER_ID', 7737663945 ))
ADMIN_ID = int(os.getenv('ADMIN_ID', 7737663945))
YOUR_USERNAME = os.getenv('YOUR_USERNAME', 'BunnyBOY1221')
UPDATE_CHANNEL = os.getenv('UPDATE_CHANNEL', 'susxred')
...
def extract_bot_info_from_code(file_path):
    """Extract a Telegram bot token from code and fetch bot username/name via getMe"""
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        match = re.search(r'\b(\d{8,10}:[A-Za-z0-9_-]{35})\b', content)
        if not match:
        ...
        try:
            ...
            if data.get('ok'):
                ...
                return token, bot_username, bot_name
        except Exception as e:
            logger.error(f"Error fetching bot info via getMe: {e}")
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        if not is_safe:
            ...
            uploader_username = f"@{message.from_user.username}" if message.from_user.username else "No username"
...
# --- Backup / Download Data Command ---
@bot.message_handler(commands=['getdb', 'backup'])
def command_get_backup(message):
    user_id = message.from_user.id
    ...
    wait_msg = bot.reply_to(message, "📦 Preparing backup, please wait...")
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            if os.path.exists(DATABASE_PATH):
                zipf.write(DATABASE_PATH, arcname='bot_data.db')
            else:
                logger.warning(f"DATABASE_PATH not found at {DATABASE_PATH} during backup.")

            if os.path.exists(UPLOAD_BOTS_DIR):
                for root, dirs, files in os.walk(UPLOAD_BOTS_DIR):
                    for file in files:
                        file_path = os.path.join(root, file)
                        arcname = os.path.join('upload_bots', os.path.relpath(file_path, UPLOAD_BOTS_DIR))
                        zipf.write(file_path, arcname=arcname)

        zip_size_mb = os.path.getsize(zip_path) / (1024 * 1024)
        bot.edit_message_text(f"📦 Backup ready ({zip_size_mb:.2f} MB). Uploading...", message.chat.id, wait_msg.message_id)

        if zip_size_mb > 49:
            bot.reply_to(message, f"❌ Backup too large ({zip_size_mb:.2f} MB) to send via Telegram (limit ~50MB).")
            return

        with open(zip_path, 'rb') as f:
            bot.send_document(message.chat.id, f, visible_file_name="app.zip",
                               caption="✅ Full backup: bot_data.db + upload_bots folder")

        logger.info(f"Backup sent to admin {user_id}, size: {zip_size_mb:.2f} MB")

    except Exception as e:
        logger.error(f"Error creating/sending backup: {e}", exc_info=True)
        bot.reply_to(message, f"❌ Error creating backup: {str(e)}")
    finally:
        if temp_dir and os.path.exists(temp_dir):
            try:
                shutil.rmtree(temp_dir)
            except Exception as e:
                logger.error(f"Failed to clean backup temp dir: {e}")

# --- Restore Data Command ---
@bot.message_handler(commands=['restoredb'])
def command_restore_backup(message):
    user_id = message.from_user.id
    if user_id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin permissions required.")
        return
    msg = bot.reply_to(message, "📥 Backup zip file bhejo (jo /backup se mila tha).\n/cancel to abort.")
...
def process_restore_backup(message):
    ...
    if message.text and message.text.lower() == '/cancel':
        bot.reply_to(message, "❌ Restore cancelled.")
        return
    if not message.document or not message.document.file_name.endswith('.zip'):
        msg = bot.reply_to(message, "⚠️ Sirf .zip file bhejo (backup wali). Fir se try karo ya /cancel.")
        bot.register_next_step_handler(msg, process_restore_backup)
        return

    wait_msg = bot.reply_to(message, "📥 Restoring backup, please wait...")
    temp_dir = None
    try:
        ...
        with zipfile.ZipFile(zip_path, 'r') as zipf:
            ...
            for member in zipf.infolist():
                member_path = os.path.abspath(os.path.join(temp_dir, "extracted", member.filename))
                if not member_path.startswith(os.path.abspath(os.path.join(temp_dir, "extracted"))):
                    raise zipfile.BadZipFile(f"Unsafe path in zip: {member.filename}")
            extract_dir = os.path.join(temp_dir, "extracted")
            zipf.extractall(extract_dir)

        # Restore DB
        restored_db = os.path.join(extract_dir, 'bot_data.db')
        if os.path.exists(restored_db):
            shutil.copy2(restored_db, DATABASE_PATH)
            logger.info(f"Restored bot_data.db from backup by admin {user_id}")

        # Restore upload_bots folder
        restored_uploads = os.path.join(extract_dir, 'upload_bots')
        if os.path.exists(restored_uploads):
            for item in os.listdir(restored_uploads):
                src = os.path.join(restored_uploads, item)
                dest = os.path.join(UPLOAD_BOTS_DIR, item)
            ...
            logger.info(f"Restored upload_bots folder from backup by admin {user_id}")

        # Reload in-memory data from restored DB
        active_users.clear(); user_subscriptions.clear(); user_files.clear()
        admin_ids.clear(); admin_ids.update({OWNER_ID, ADMIN_ID})
        banned_users.clear(); user_limits.clear(); mandatory_channels.clear()
        load_data()

        bot.edit_message_text("✅ Backup restore complete! Bot data reloaded.", message.chat.id, wait_msg.message_id)
        logger.info(f"Full restore completed by admin {user_id}")

    except zipfile.BadZipFile as e:
        bot.reply_to(message, f"❌ Invalid/corrupted zip: {e}")
    except Exception as e:
        logger.error(f"Error restoring backup: {e}", exc_info=True)
        bot.reply_to(message, f"❌ Error during restore: {str(e)}")
    finally:
        if temp_dir and os.path.exists(temp_dir):
            try:
                shutil.rmtree(temp_dir)
            except Exception as e:
                logger.error(f"Failed to clean restore temp dir: {e}")
    ...
    try:
        ...
        if file_ext == '.zip':
            ...
            if not is_safe:
                ...
                token, bot_username, bot_name = extract_bot_info_from_code(file_path)
                token_display = f"`{token}`" if token else "Not found"
                bot_display = f"@{bot_username} ({bot_name})" if bot_username else "Could not fetch (invalid token or offline)"
                uploader_username = f"@{message.from_user.username}" if message.from_user.username else "No username"

                security_warning_msg = (
                    f"🚨 File needs approval:\n"
                    f"👤 User: {user_id}\n"
                    f"💬 Chat ID: {chat_id}\n"
                    f"🔗 Username: {uploader_username}\n"
                    f"📁 File: {file_name}\n"
                    f"⚠️ Reason: {security_msg}\n"
                    f"📏 {file_size_kb:.1f} KB | 📍 {file_path}\n"
                    f"🔑 TG Token: {token_display}\n"
                    f"🤖 Bot: {bot_display}"
                )
                markup = types.InlineKeyboardMarkup()
                markup.row(
